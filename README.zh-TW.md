...
<p align="center">
  <strong>讓 coding agents 跟著專案一起保持對齊。</strong>
</p>
...
從 agent 的角度看，Crabyard 最實際的價值，是把原本只能靠聊天回想的事情，變成幾個可以直接查詢的答案。它不用再每次重新拼湊上下文，而是能先確認這個變更是否站得住腳，再決定下一步。

實際上，它拿到的是：

- 可預測的變更包驗證
- 用來看專案與變更狀態的 `status --json`
- 目前哪些單元可開始、哪些被擋住的執行範圍資訊
- 有明確型別的驗證資料，並相容舊式簡寫
- 安裝在 `.agents/skills/` 下、只屬於這個專案的 skills

## 為什麼和 Codex / OpenCode 配起來會更好

沒有 Crabyard：

```text
你下提示 -> agent 改程式 -> 你看 diff -> 再下提示一次 -> 希望上下文還對得上
```
...
- 意外修改到所有權之外檔案的機率更低
- 規劃、實作、審查之間更容易交接
- 更清楚知道「agent 下一步到底該做什麼」

## 設計目標

Crabyard 是刻意做小的。

它不想變成一個很大的框架、外掛市集，或一整套 AI 開發作業系統。想法其實很單純：替單一專案建一層清楚的工作記憶、清楚的執行規範，以及一種能把經驗真正留下來的方式。

這代表可長期保留的上下文要盡量留在專案內，人類可讀的規劃和機器可檢查的執行狀態要分開，而且你要能很直接地看出什麼可以開始、什麼被阻擋、什麼可以安全結案。如果某個流程讓這些事情變得更混亂，那它就不該放進來。

## Crabyard 的定位差異

Compound Engineering 和 OpenSpec 都在這個方向上做了很有價值的事情，Crabyard 也確實受到它們啟發。共同的核心判斷是對的：當專案本身有更多結構，AI 輔助開發會穩很多。

真正的差異主要在範圍。

Compound Engineering 比較大，也更像一個生態系。它涵蓋更多工具、更多工作流程，以及更多可重用流程。這樣做有它的力量。Crabyard 想解的是另一種問題：當你不想再增加需要維護的面向，而是想讓單一專案的工作方式更清楚、更容易被 agent 穩定掌握時，應該怎麼做。

OpenSpec 在精神上更接近，但它仍然是比較完整的規格框架。Crabyard 則刻意再收斂一點。很多團隊真正缺的不是更多流程階段，而是更少的變動元件、更強的執行事實，以及一套不會讓已接受的產品知識再次散回聊天記錄的做法。

所以 Crabyard 的主張其實很直接：如果你要的是更容易管理的上下文、更容易維持的工作流程，還有一個會跟著專案一起變強的結構，那它就是刻意做成這個比較小、比較穩的樣子。
...
你可以把 Crabyard 想成是放在一般 agent 對話旁邊的一層共享工作記憶與執行規範。你和 Codex 或 OpenCode 的互動方式不需要改很多，真正改變的是：專案本身終於有地方可以把規劃、執行範圍與結案條件說清楚。

典型使用方式：

```text
1. 你要求 Codex/OpenCode 做一個功能或修復
...
Crabyard 會在 `.agents/skills/` 下安裝一組很小的專案內 skills。這是刻意的。你應該可以複製一個專案、跑完 `init`，然後立刻給 agent 同一套小而穩的工具，而不是還要依賴某個人的全域安裝。
...
2. 要求 Codex/OpenCode 建立 `crabyard/changes/<slug>/`
