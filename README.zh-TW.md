...
`init` 用來第一次安裝專案內的 Crabyard 結構。CLI 升級後，使用 `refresh` 更新可安全替換的 Crabyard 管理資產，例如專案內 skills，以及 `AGENTS.md` 中由 Crabyard 管理的路由區塊。

`refresh` 會把目前安裝的 CLI 版本寫進 `crabyard/manifest.yaml` 的 `managed_by.crabyard_version`。`project.md`、`knowledge/index.md`、`TASK_EXECUTION_FORMAT.md`、各 bucket 的 `README.md` 這些由 repo 自己維護的文件會被保留，只有缺檔時才補回。舊的 `update` 指令仍保留為 alias。

只有在你真的想把被取代的 Crabyard 管理檔案複製到 `.crabyard/backups/` 時，才加上 `--backup`。

在 Codex、opencode，或其他支援專案內 Skills 的 agent CLI 裡，請讓 agent 使用 Crabyard skills，而不是手動替它操作每一個 CLI 指令：

- `crabyard-research`：在重大決策前取回既有專案知識
- `crabyard-explore`：檢查 repo，釐清變更範圍
- `crabyard-plan`：建立或更新 `crabyard/changes/<slug>/`
- `crabyard-apply`：從目前可執行範圍開始實作
- `crabyard-review`：審查規劃或實作
- `crabyard-archive`：替已驗證且同步一致的變更結案
- `crabyard-debug`、`crabyard-learn`、`crabyard-refresh`：用在除錯或維護專案知識時

一次完整的變更流程仍會用到 `check`、`verify`、`sync` 與 `archive`；skills 會以 CLI 作為權威來源來處理這個流程。
...
- `crabyard check`：實際執行變更裡正規化後的驗證資料
...
你可以把 Crabyard 想成是放在一般 agent 工作流程旁邊的一層共享工作記憶與執行規範。實際互動通常像這樣：
...
- `install`：`init` 的別名
- `refresh`：更新既有 repo 中可安全替換的 Crabyard 管理資產，同時保留 repo 自己維護的文件
- `update`：`refresh` 的別名
...
- 不帶額外 target 的 `validate` 會檢查 repo 結構、由 Crabyard 管理的 memory block，以及所有進行中的變更
- `validate change <name>` 會只檢查單一 change bundle
- 文字與 JSON 輸出現在都會包含 Crabyard 管理資產的版本狀態，方便在驗證時直接看到專案內 skills 或路由資產是否落後於目前安裝的 CLI
...
- `status` 也會回報專案內的 Crabyard 管理資產是否和目前安裝的 CLI 版本一致；不一致時會直接給 refresh 提示
...
既有 repo 先執行 CLI 指令 `crabyard refresh <repo>` 更新專案內由 Crabyard 管理的 skills。這個指令不會重寫 repo 自己維護的知識。若舊知識需要整理，再用 `crabyard-refresh` skill 搭配窄範圍，讓 agent 只更新已經過時或重複的檢索目標。
